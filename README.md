...
**`場次總表`** — 所有場次的索引
`場次代碼 | 場次名稱 | 狀態 | 目前階段 | 招募開放 | 彩池階段 | 彩池剩餘 | 主持通行碼 | 建立時間 | 更新時間 | 聘書已發放`
...
**玩家**：首頁 →「我是玩家」→ 輸入主持人給的日期 → **填暱稱**並選角入場
→ 之後畫面每 3 秒自動更新，隨時看到自己的威望值、勢力值與排名
→ 聘書發放後會出現在「我的」分頁最上方。

暱稱是聘書上的署名，入場時必填（20 字以內）。改版前入場、沒有暱稱的玩家，
可以在「我的」分頁的場次資訊裡按「填寫」補上；聘書一發放就鎖住不能再改。
...
npm run test:api              # 暱稱、招募、聘書、陣營、動態可見性、拍賣（84 個案例）
...
`session:remove` 會刪掉指定場次的 `_玩家`／`_紀錄`／`_舉報` 三個分頁與總表那一列，
