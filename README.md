...
結束不代表資料就消失了——**玩家用原本那支手機打開，仍然看得到自己的紀錄**：
最終勢力值與威望值、聘書、自己的那幾筆流水帳，以及故事復盤。

這個畫面是唯讀的，資料從彙整分頁讀回來（工作分頁已經刪了，那是唯一的來源）。
主持人按下「結束」的當下，玩家端還開著的畫面會自己切換過去，不用重新整理。

開場密碼在場次結束後**仍然找得到那一場**，但只能回顧不能進場。
同一組密碼拿去開新場時，一定會進到新的那一場——不會掉回舊的回顧。

主持台也一樣：結束後畫面不會壞掉，會顯示「已封存」並提示資料在哪個分頁。

⚠️ 回顧靠的是瀏覽器裡存的身分。**換手機或清掉瀏覽器資料就找不回來了**——
場次已經結束，沒辦法再用暱稱認回（那個機制只在進行中的場次有效）。
真的需要的話請主持人查試算表。
...
早期沒有`通行碼`欄的封存，改用玩家代碼單獨比對（代碼存在各自的瀏覽器裡、
介面上從不顯示）。強度低一點，但總比那一場的人完全看不到自己的紀錄好。
...
`tests/review.test.mjs` 驗散場後的回顧：封存後即時狀態拿不到但回顧讀得到、
成績與聘書原樣保留、只看得到自己的紀錄、通行碼不對讀不到、
故事復盤在封存後仍然看得到、密碼仍找得到舊場次但同一組密碼開新場時會指向新的、
早期沒有通行碼欄的封存也讀得回來。
