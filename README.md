...
| 玩家掉線後用暱稱認回自己的角色，遊戲中途也適用 | ✅ |
| 場次結束後玩家仍可回來看自己的成績、聘書與紀錄 | ✅ |
...
| 玩家之間互相轉贈勢力值 | ✅ |
| 第一週競選投票：同意 2 票／不同意 1 票，全員投完才能進下一階段 | ✅ |
...
封存之後那一場就不能再進場、調配或重新開放了。

### 散場後的個人回顧

結束不代表資料就消失了。玩家再打開這個場次，會看到一份唯讀的回顧：
最終勢力值與威望值、聘書、自己的那幾筆流水帳，以及故事復盤。

**這裡不做身分驗證**——直接列出角色讓人挑，選了就看得到。理由是場次結束後
什麼都不能操作，只剩紀錄可看；而且玩家換過手機、清掉瀏覽器資料之後，
本來就沒有身分可驗了。瀏覽器裡還留著身分的話會直接開那個角色，也可以按「換角色」。

⚠️ 代價是**同場的人可以互相看到對方的紀錄**，包括誰投了誰的不同意票、誰舉報了誰。
散場之後這通常正是大家在桌上聊的內容，而且故事復盤本來就把全部真相攤開了——
但如果哪天想改回只能看自己的，`getReviewSnapshot` 加回通行碼比對即可
（彙整分頁裡有存）。

主持人按下「結束」的當下，玩家端還開著的畫面會自己切換過去，不用重新整理。
主持台也不會壞掉，會顯示「已封存」並提示資料在哪個分頁。

開場密碼在場次結束後**仍然找得到那一場**，但只能回顧不能進場。
同一組密碼拿去開新場時，一定會進到新的那一場——不會掉回舊的回顧。

#### 彙整分頁的欄位是會變的

解析彙整分頁時一律**依欄位名取值**，不要寫死位置。`通行碼`就是後來才補進去的——
寫死位置的話，早期的封存讀起來會整排錯開，而且是靜默錯開，
顯示出來的每個數字都是別欄的值。

`通行碼`目前只是存著備用——回顧不驗身分，但欄位留著，哪天要改回驗證就不用再改格式。
...
玩家的身分存在瀏覽器裡。**換手機、清掉瀏覽器資料、分頁被系統回收之後身分就沒了**，
而他的角色已經被自己選走——沒有回場機制的話，那個人整場就回不去了。

回場方式：在選角畫面**點自己原本的角色**（已被選走的也點得下去），
輸入**入場時填的暱稱**即可，數值、技能卡、招募次數都還在。

- 暱稱比對會忽略前後空白與英文大小寫
- 打錯只會說「暱稱不正確」，不會洩漏正確答案
- **遊戲進行到一半也能回場**——那正是最需要的時候。這跟「新玩家入場」是兩件事，
  後者仍然照階段規則擋住
- 忘記自己填什麼的話，主持人在「設定」分頁看得到每個人的暱稱
...
「指向別人」跟「不在名單中」是**兩種不同的待遇**，很容易寫反：

| 輸入的編號 | 結果 |
|---|---|
| 21 張之一，且指向被舉報人 | 舉報成立 → 結算時**被舉報人** −1 威望，該線索卡對全場公開 |
| 21 張之一，但指向別人 | 當場回「您輸入錯誤」→ **不受理也不留紀錄**，可以改對象再送 |
| 不在 21 張之中 | 照常受理 → 結算時**舉報人** −1 威望 |

也就是說：拿著真卡指錯人只算填錯，可以重來；隨便編一個編號才要付代價。

單次使用只對**真的**線索卡有意義。編造的編號不做這個檢查——否則第二個人輸入
同一串會被告知「已被使用」，等於洩漏有人送過。

結果一律等到主持人切換下一階段才公布，而且只顯示數字、不顯示明細。

---

### 第一週：競選會長助理

玩家在「行動」分頁投票，每人**同意票 2 張、不同意票 1 張**。

- 不能投自己
- 三張票要投給**三個不同的人**（投過的人會從名單上收起來）
- 投出去不能收回
- 場上人數不足時票數自動下修（3 人場只有 2 張同意票），
  否則「全員投完」的閘門會永遠卡住

主持人在「舉報」分頁看得到**每個人還剩幾張沒投**，但看不到誰投給誰——
票型只出現在「紀錄」分頁。**全員投完才能切換到下一階段**，
因為結算要靠完整票型，缺一張結果就不對。

有人臨時離場投不了的話，把他從「設定」分頁移出場次，進度就不會再等他。

離開第一週時自動結算：

- 一張票 **1 點威望**，同意加、不同意減
- 結算後**威望最高**的人自動獲得「當選會長助理」**+200 勢力**
  （平票比勢力值，再平就比入場順序）

結算綁在「離開第一週」而不是「進入第二週」，所以主持人若直接跳到第三週也不會漏算。
...
npm run test:api              # 場次、回場、聘書、陣營、動態、投票、舉報、拍賣、復盤、回顧（227 個案例）
...
`tests/review.test.mjs` 驗散場後的回顧：不帶身分拿得到角色名單（且名單不含真實陣營）、
挑了角色就讀得到成績與聘書、每個人的紀錄各自獨立、不存在的角色代碼會被擋、
故事復盤在封存後不驗身分也看得到、沒發聘書的場次沒有復盤、
密碼仍找得到舊場次但同一組密碼開新場時會指向新的、早期格式的封存也讀得回來。

`tests/story.test.mjs` 驗故事復盤：聘書發放前誰都拿不到（而且擋下時不夾帶任何內容）、
沒有身分或通行碼不對拿不到、發放後全員可看、七個角色的陣營與劇本一致、
內容涵蓋各角色的真實身份與結局。

`tests/report.test.mjs` 驗舉報判定：真卡指錯人被退回且不留紀錄、改對對象就能送出、
名單外的編號照樣受理、同一串編造編號別人也送得出去（不然會洩漏有人送過）、
真卡仍然單次使用、結算後三筆懲罰各自落在正確的人身上、編造的編號不會被公開。

`tests/vote.test.mjs` 驗競選投票：票數限制、不能投自己、同一人不能投兩次
（連換票種也不行）、沒投完擋下換階段、票型只有主持人看得到、
結算後的威望加減正確、會長助理的三層排序（威望→勢力→入場順序）、
人數不足時票數自動下修。
